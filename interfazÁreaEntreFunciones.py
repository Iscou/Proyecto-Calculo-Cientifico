...
from logicaÁreaEntreFunciones import calculo_final # Nuestro .py que hace el calculo del area entre curvas
...
def capturarCoordendas(evento):
    ...
    if not (0 <= x_px <= 600 and 0 <= y_px <= 400):
        return 
    ...
    if fase_actual == "esperando_imagen":
        ...
        if x_max_px <= x_min_px or y_min_px >= y_max_px:
            solicitudDimesionesRectangulo.config(text=" Error: La esquina superior debe estar arriba y a la derecha. Intenta de nuevo (Clic inferior izq).")
            lienzo.delete("all")
            if hasattr(root, 'imagen_tk'):
                lienzo.create_image(0, 0, anchor=tk.NW, image=root.imagen_tk)
            fase_actual = "calibrando_p1"
            return
        ...
        if len(extremos_f) > 0 and xReal <= extremos_f[-1][0]:
            solicitudDimesionesRectangulo.config(text=" Marca los extremos de Izquierda a Derecha estrictamente.")
            return
        ...
        x_izq = extremos_f[tramo_actual_f][0]
        x_der = extremos_f[tramo_actual_f + 1][0]
        if not (x_izq < xReal < x_der):
            solicitudDimesionesRectangulo.config(text=f" El nodo debe estar entre x={x_izq:.1f} y x={x_der:.1f}")
            return
        ...
        if len(extremos_g) > 0 and xReal <= extremos_g[-1][0]:
            solicitudDimesionesRectangulo.config(text=" Marca los extremos de Izquierda a Derecha estrictamente.")
            return
        ...
        x_izq = extremos_g[tramo_actual_g][0]
        x_der = extremos_g[tramo_actual_g + 1][0]
        if not (x_izq < xReal < x_der):
            solicitudDimesionesRectangulo.config(text=f" El nodo debe estar entre x={x_izq:.1f} y x={x_der:.1f}")
            return
